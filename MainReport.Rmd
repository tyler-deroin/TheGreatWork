...
outside_london <- fdata$binary_marstat[fdata$region != "Inner London"]
t.test(fdata$binary_marstat, fdata$binary_marstat[fdata$region == "Inner London"])
t.test(outside_london, fdata$binary_marstat[fdata$region == "North East"])
t.test(outside_london, fdata$binary_marstat[fdata$region == "North West"])
t.test(outside_london, fdata$binary_marstat[fdata$region == "Yorkshire and the Humber"])
t.test(outside_london, fdata$binary_marstat[fdata$region == "East Midlands"])
t.test(outside_london, fdata$binary_marstat[fdata$region == "West Midlands"])
t.test(outside_london, fdata$binary_marstat[fdata$region == "East of England"])
t.test(outside_london, fdata$binary_marstat[fdata$region == "Outer London"])
t.test(outside_london, fdata$binary_marstat[fdata$region == "South East"])
t.test(outside_london, fdata$binary_marstat[fdata$region == "South West"])
t.test(outside_london, fdata$binary_marstat[fdata$region == "Wales"])
...
fd %>% dplyr::select(region, binary_marstat, Highest_qualification, binary_region_factor)%>% filter(!is.na(Highest_qualification), !(Highest_qualification == 'Others')) %>%group_by(region, Highest_qualification)%>% summarise(divorce_rate = mean(binary_marstat)) %>% ggplot(aes(x = region, y = divorce_rate, fill = Highest_qualification))+ geom_col(position = 'dodge')+ theme(axis.text.x = element_text(angle = 60, hjust = 1)) + labs(title = 'Divorce rate by qualifications across regions')
...
fd %>% dplyr::select(region, marstat, hours) %>% group_by(region, hours) %>% summarise(count = n())%>%
...
```{r}
tenure_data %>% dplyr::select(region, binary_marstat, tenure) %>% group_by(region, tenure) %>% summarise(count = n())%>%
...
```{r}
tenure_data %>% dplyr::select(binary_marstat, tenure) %>% group_by(tenure) %>%summarise(divorce_rate = mean(binary_marstat)) %>% ggplot(aes(x = tenure, y = divorce_rate))+ geom_col() +theme(axis.text.x = element_text(angle = 60, hjust = 1)) + labs(title = 'Divorce rate by tenure')
```

Now compare the divorce rate across regions and tenure. The divorce rates 

```{r}
tenure_data %>% dplyr::select(region, binary_marstat, tenure)%>% group_by(region, tenure)%>% summarise(divorce_rate = mean(binary_marstat)) %>% ggplot(aes(x = tenure, y = divorce_rate, fill = tenure))+ geom_col() + scale_fill_manual(values = c('gray50', 'gray50','gray50', 'red','gray50','gray50'))+ facet_wrap(~region)+ theme(axis.text.x = element_text(angle = 60, hjust = 1)) + labs(title = 'Tenure by region')
```
...
```{r}
tenure_data <- filter(fd, !is.na(tenure))



tenure_data %>% dplyr::select(binary_marstat, tenure, region) %>% group_by(region, tenure) %>%summarise(divorce_rate = mean(binary_marstat)) %>% ggplot(aes(x = region, y = divorce_rate,  fill = tenure))+ geom_col(position = 'dodge')+theme(axis.text.x = element_text(angle = 60, hjust = 1)) + labs(title = 'Divorce rate by tenure across region')
```
...
```{r}
fulldata %>% dplyr::select(region, marstat, tenure) %>% filter(marstat != 'Single' & marstat != 'Widowed')%>%group_by(region, marstat, tenure) %>% summarise(count = n())%>%
  ungroup() %>% group_by(region, marstat)%>%mutate(prop = count /sum(count)) %>% ggplot(aes(x = tenure, y = prop, fill = tenure))+ geom_col() + scale_fill_manual(values = c('gray50', 'gray50','gray50', 'red','gray50','gray50')) + facet_grid(marstat~region)+theme(axis.text.x = element_blank()) + labs(title = 'Tenure by marital status')
```
