...
fd %>% dplyr::select(region, marstat, hours) %>% group_by(region, hours) %>% summarise(count = n())%>% ungroup() %>% group_by(region)%>%mutate(prop = count /sum(count)) %>% ggplot(aes(x = hours, y = prop))+ geom_col() + facet_wrap(~region)+theme(axis.text.x = element_text(angle = 30, hjust = 1)) + labs(title = 'Hours worked by region')
```
