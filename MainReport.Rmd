...
pie(mydata_relig$count, labels = mydata_relig$religionew, col=rainbow(length(lbls2)), main="Pie chart of religions") + pie(mydata_ethn$count, labels = mydata_ethn$ethngrouped, col=rainbow(length(lbls)), main="Pie chart of ethnicities")
```


Our assumption is that divorce rate will be higher among white people who are generally considered having more 'liberal' values, and permissive laws for divorce. We also expect that the less commited people are toward monotheist religions, the more they are likely to get divorced.
