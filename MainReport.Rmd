...
Ethnicites/Religion/Countries 

It cannot be denied that the cultural package each person has, is a major factor in his conception of marriage, his expectations toward a relationship and his values about divorce. Thus, in this part, we examine the factors of Ethnicity, Religion and Country of birth to try to determine the effect it has on divorce rate. Our assumption is that divorce rate will be higher among white people who are generally considered having more 'liberal' values, and permissive laws for divorce. e also expect that the less commited people are toward monotheist religions, the more they are likely to get divorced.
...
```{r, echo = FALSE}
# Correlation tests
cor.test(myowndata$religionew, myowndata$binary_marstat)

cor.test(myowndata$ethnicityew, myowndata$binary_marstat)

cor.test(myowndata$cobg, myowndata$binary_marstat)
```
...
#ggplot(data=ethn) + geom_bar(aes(x = binary_marstat, y = prop, fill = binary_marstat), stat = 'identity', position = position_dodge())  + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + labs(title = 'ethnicity') + facet_wrap(~ ethnicityew)

ethn_divorced = filter(ethn, binary_marstat == "Separated/Divorced")
ethn_divorced <- ethn_divorced[order(ethn_divorced$prop),]

#ggplot(data=ethn_divorced) + geom_bar(aes(x = reorder(ethnicityew, prop), y = prop), stat = 'identity', position = position_dodge()) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + labs(title = 'Ethnicity')
```
```{r, echo = FALSE}
...
The divorce rate show us some insight that we expected. For example, Asian divorce rate is significantly lower that for White people. The divorce rate is also very high for a person of mixed ethnicities. This might be explained by the fact that a mixed couple requires a more considerable amount of adjustment, tolerance and understandings and is more likely to divorce (http://ftp.iza.org/dp5960.pdf). Parents' value about divorce may thus transmitted to the child.
However, one unexpected result is that divorce rate are the highest among black people.

Find article for uk or compare to us (http://www.swaraylawoffice.com/2010/08/divorce-rate-amongst-african-immigrants/)
## Religion
```{r, echo = FALSE}
...
```
Cross-checking the religion and ethnicities variables, we can observe that the religion with the highest divorce rate are 'Other religion', 'No religion' or 'No stated', which show more liberal values.
However Buddhist also have high divorce rate, probably due to its permissive point of view toward divorce (https://www.budsas.org/ebud/whatbudbeliev/237.htm).
Finally, the next religion with the highest divorce rate is Christianism which is not very surprising. Indeed, even though marriage is a sacred union in the religion, Europeans tend to be less and less committed or involved in religions, and even christianism' strictness toward divorce is changing.
...
We can see that the pattern of the countries of birth has some similarities with the world divorce rate heatmap you can find following this link: http://i.imgur.com/dep7XIZ.png.
