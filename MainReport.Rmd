...
fdata[,'marstat'] <- factor(fdata[,'marstat'], level = c(1,2,3,4,5,6), 
...
We continue to explore other variables in our list and see if we could find any pattern. 

## Type of dwelling and region

When we compare the types of dwelling across regions, inner London has the highest percentage of people staying in rented property. 
```{r}
...
### Type of dwelling and marital status
...
```{r}
fulldata %>% dplyr::select(marstat, tenure) %>% filter(marstat != 'Single' & marstat != 'Widowed')%>%group_by(marstat, tenure) %>% summarise(count = n())%>%
  ungroup() %>% group_by(marstat)%>%mutate(prop = count /sum(count)) %>% ggplot(aes(x = tenure, y = prop))+ geom_col() + facet_wrap(~marstat)+theme(axis.text.x = element_text(angle = 60, hjust = 1)) + labs(title = 'Tenure by marital status')
...
## To further find out if types of dwelling affects the marital status, we look at London data alone

```{r}
London_rent <- fulldata %>% select(region, tenure, marstat, Highest_qualification) filter(fulldata, region == 'Inner London' & tenure == 'Rents')
...
family_compo <- fulldata %>% select(dpcfamuk11, tenure, marstat, region) %>% filter(marstat != 'Single' & marstat != 'Widowed')%>% mutate(no_of_children = dpcfamuk11)%>% mutate(children_class = ifelse(dpcfamuk11==1, 'No children', ifelse(dpcfamuk11==2|dpcfamuk11==3|dpcfamuk11==4|dpcfamuk11==5|dpcfamuk11==6|dpcfamuk11==7, 'One child', ifelse(dpcfamuk11==8|dpcfamuk11==9|dpcfamuk11==10|dpcfamuk11==11|dpcfamuk11==12|dpcfamuk11==13, 'Two children', ifelse(dpcfamuk11==14|dpcfamuk11==15|dpcfamuk11==16|dpcfamuk11==17|dpcfamuk11==18|dpcfamuk11==19, 'Three or more children', NA)))))


family_compo %>% dplyr::select(children_class, tenure, marstat)%>% filter(marstat != 'Single' & marstat != 'Widowed') %>%group_by(children_class, tenure) %>% summarise(count = n())%>%
  ungroup() %>% group_by(children_class)%>%mutate(prop = count /sum(count)) %>% ggplot(aes(x = children_class, y = prop, fill= tenure))+ geom_col(position = 'dodge') +theme(axis.text.x = element_text(angle = 60, hjust = 1)) + labs(title = 'Tenure by dependent children')
